...
async function getLatestPosts() {
  try {
    const res = await fetch(`${API_URL}/posts?limit=4`, { next: { revalidate: 600 } });
    const json = await res.json();
    return json.data ?? json ?? [];
  } catch { return []; }
}
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 space-y-12">
        ...
        {posts.length > 0 && (
          <section>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-xl font-bold text-gray-900">Güncel İçerikler</h2>
              <Link href="/blog" className="text-sm text-blue-600 hover:underline">Tüm yazılar →</Link>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {posts.map((post: any) => (
                <Link
                  key={post.id}
                  href={`${post.postType === 'news' ? '/haber' : '/blog'}/${post.slug}`}
                  className="bg-white border rounded-xl p-5 hover:shadow-md hover:border-blue-300 transition group flex flex-col"
                >
                  <span className="text-[11px] font-semibold uppercase tracking-wide text-blue-600 mb-2">
                    {post.postType === 'news' ? 'Haber' : 'Blog'}
                  </span>
